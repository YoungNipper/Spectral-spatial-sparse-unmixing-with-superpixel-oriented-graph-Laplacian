...
options.t =0.3;%这里调节权值的大小。可调参数2，相似度度量大小。
options.k =7;% k=0,全连接图 。可c调参数1，近邻的个数


parameter.AL_iters1=60;
parameter.AL_iters2=5; 
parameter.mu = 0.1;
parameter.trueX=XT;
parameter.im_size=[100,100];
parameter.num_sup=400;%这里取350，400



lambda_l1_ =  [5e-3 ];%稀疏约束
lambda_lap_ = [1e-3];% lap约束
% lambda_l1_=[0.01];
% lambda_lap_ =[ 1e-3];
a=length(lambda_l1_);
